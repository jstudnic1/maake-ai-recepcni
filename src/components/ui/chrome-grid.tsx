...
const Box = ({ 
  ...
  useEffect(() => {
    if (meshRef.current) {
      ;(meshRef.current as any).userData.gridPosition = gridPosition
    }
  }, [gridPosition])
...
function GridOfBoxes() {
  ...
  return (
    <>
      <HoverDetector gridSize={gridSize} spacingX={spacingX} spacingZ={spacingZ} onHoverChange={setHoveredBox} />
      {boxes}
    </>
  )
}
...
export function ChromeGrid() {
  return (
    <div className="chrome-grid-react">
      <Canvas
        camera={{ position: [-9.31, 12, 24.72], rotation: [-0.65 as any, -0.2 as any, -0.13 as any], fov: 35 }}
